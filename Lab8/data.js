...
MongoClient.connect(fullMongoUrl)
    .then(function(db){
        ...
        //lab8.2.2
        exports.createLog = function(requestId, requestPath, requestMethod, cookies){
            if(!requestPath) Promise.reject("Missing RequestPath");
            if(!requestMethod) Promise.reject("Missing RequestMethod");
            if(!cookies) Promise.reject("Missing Cookies");

            return logCollection.insertOne({_id: Guid.create().toString(), requestId: requestId, requestPath: requestPath, requestMethod:requestMethod}).then(function(log){
